...
def download_magnet(magnet: str, select_files: str = "") -> list:
    ...
    # Scan for external subtitle files (.srt, .ass, .ssa, .sub) that came
    # with the torrent — these will be matched to .mkv files by episode number
    sub_exts   = (".srt", ".ass", ".ssa", ".sub", ".vtt")
    all_subs   = glob.glob("**/*", recursive=True)
    ext_subs   = [f for f in all_subs
                  if os.path.splitext(f)[1].lower() in sub_exts
                  and os.path.getsize(f) > 100]  # ignore empty/stub files

    if ext_subs:
        print(f"  External subtitle files found: {len(ext_subs)}")
        for s in ext_subs:
            print(f"    {s}")

    print(f"  Valid .mkv files: {valid or 'none'}")
    return valid, ext_subs
...
def _ep_from_path(path: str) -> int | None:
    """Extract episode number from a file path (same logic as parse_file_info)."""
    base = os.path.basename(path)
    m = re.search(r"Detective Conan\s*[-–]\s*(\d{3,4})", base, re.IGNORECASE)
    if m:
        return int(m.group(1))
    # Zero-padded 3-digit e.g. "0174" → 174
    m = re.search(r"0*(\d{3,4})", base)
    if m:
        return int(m.group(1))
    return None


def build_subtitle_map(sub_files: list) -> dict:
    """
    Build a dict mapping episode_number → best subtitle file path.

    Priority: .ass/.ssa (styled) > .srt > .sub > .vtt
    When multiple subs exist for the same episode, prefer English-tagged ones.
    """
    ext_priority = {".ass": 0, ".ssa": 1, ".srt": 2, ".sub": 3, ".vtt": 4}
    sub_map: dict[int, tuple] = {}  # ep → (priority, path)
...
def _find_english_subtitle_index(input_file: str) -> int:
    ...
    try:
        ...
        if not streams:
            # No subtitle streams at all — signal hardsub to skip
            return -1
...
def hardsub(input_file: str, label: str,
            external_sub: str | None = None) -> str | None:
    """
    Burn subtitles into video using ffmpeg.

    If external_sub is provided (.srt/.ass from torrent), uses it directly.
    Otherwise scans embedded subtitle streams via ffprobe and picks English.
    ...
    for vf in vf_list:
mation_dood_only.py — Detective Conan Single DoodStream Automation
...
def process_file(mkv_file: str):
    ...
    # Look up any external subtitle that came with this file's torrent
